...
export class BooksService {
  ...
  async searchBooks(query: string, limit = 20, page = 1) {
    ...
    const items = pageRecords.map((r: any) => {
      ...
      const demandPressure =
        rawDemandPressure && typeof rawDemandPressure.toNumber === "function"
          ...
          : rawDemandPressure !== undefined && rawDemandPressure !== null
            ...
            : 0;
      const highDemand =
        mapped.availableCopies < 3 && mapped.totalCopies > 0 ? true : false;
  ...
  private mapNeo4jToBook(node: any) {
    ...
    const borrowCount =
      rawBorrowCount && typeof rawBorrowCount.toNumber === "function"
    ...
    return {
      ...
      createdAt: node.properties.createdAt
        ...
        : null,
      totalCopies:
        node.properties?.totalCopies ??
        (props?.copies ? props.copies.length : undefined),
      availableCopies:
        typeof node.properties?.availableCopies !== "undefined"
          ? node.properties.availableCopies
          : props?.copies
            ? props.copies.filter((c: any) => c.status === "AVAILABLE").length
            : 0,
